...
try:
    ser = serial.Serial('COM3', 115200, timeout=1)  # Replace 'COM4' with the correct port
...
def send_quit():
    if ser and ser.is_open:
        ser.write(b'QUIT\n')
        Last_command_Label.config(text="Last Command Sent: QUIT")
    else:
...
def on_close():
    """Cleanup function to close the serial port and exit the program."""
    ser.close()  # Close the serial port
    window.destroy()  # Destroy the GUI window
