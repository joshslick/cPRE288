...
while True:  # Loop recieving/sending data from/to client until client disconnects, or error causes data to be 0 (i.e., no data)
        ...
        # Check if a sensor scan command has been sent
            ...
            move_distance = data.decode().split()[1]  # Extract the distance from the command
            print(f"Received Move command with distance: {move_distance}")
            response = f"Moving Cybot {move_distance} units\n"
